...
app.get("/pets/:pet_owner_id", (req, res) => {
  ...
  // console.log(pet_owner_id);
  const sqlQuery =
    "SELECT * FROM pets WHERE pet_owner_id = ? AND isArchived = 'False'";
...
app.get("/reservation/history/:pet_owner_id", (req, res) => {
  ...
  // console.log(pet_owner_id);
  const sqlQuery =
    "SELECT * FROM vet_clinic JOIN products ON vet_clinic.vetid = products.vetid JOIN reservation ON reservation.product_id= products.product_id JOIN pet_owners ON pet_owners.pet_owner_id = reservation.pet_owner_id WHERE reservation.pet_owner_id = ? AND reservation.reservation_status='Done'";
  db.query(sqlQuery, pet_owner_id, (err, result) => {
    // console.log(result);
    res.send(result);
  });
...
app.get("/appointments/histories/:pet_owner_id", (req, res) => {
  ...
  // console.log(pet_owner_id)
  const sqlQuery =
    "SELECT * FROM appointment INNER JOIN services ON  appointment.service_id = services.service_id INNER JOIN vet_clinic ON vet_clinic.vetid = appointment.vetid WHERE pet_owner_id = 58 AND appointment.appointment_status = 'Done' ORDER BY appointment.logs DESC";
...
app.get("/general/appointment/:vetid", (req, res) => {
  ...
  // console.log(vetid)
  const sqlQuery =
    "SELECT * FROM pet_owners JOIN pets ON pet_owners.pet_owner_id = pets.pet_owner_id JOIN appointment ON appointment.pet_id = pets.pet_id JOIN services ON services.service_id=appointment.service_id WHERE appointment.vetid = ? AND appointment.appointment_status='Approved'";
  db.query(sqlQuery, vetid, (err, result) => {
    // console.log(result);
    res.send(result);
  });
...
app.get("/history/appointment/:vetid", (req, res) => {
  ...
  // console.log(vetid)
  const sqlQuery =
    "SELECT * FROM pet_owners JOIN pets ON pet_owners.pet_owner_id = pets.pet_owner_id JOIN appointment ON appointment.pet_id = pets.pet_id JOIN services ON services.service_id=appointment.service_id WHERE appointment.vetid = '$2b$10$4RujGwsxeIvMLoeOjX6PYOShX5Xl5sA5A72yHDYdjBLjRWLyhLhbc' AND appointment.appointment_status IN ('Done', 'Decline') ORDER BY appointment.logs DESC";
  db.query(sqlQuery, vetid, (err, result) => {
    // console.log(result);
    res.send(result);
  });
